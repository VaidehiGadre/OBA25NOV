...
public class TransactionDetailsDaoImpl implements TransactionDetailsDao {
	...
	public List<Transaction_Details> allStatementsSent(String currentaccno, Date from, Date to) {
		TypedQuery<Transaction_Details> querysent = em.createQuery("select transactions FROM Transaction_Details transactions where date_of_transfer between :from and :to AND SENDER_ACC_NO=:currentaccno ",
